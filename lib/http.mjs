...
const RANGE_UNITS = new Set(['bytes']);
...
const getGenericHeaders = () => [
  ['Server', 'poteto'],
  ['Accept-Ranges', [...RANGE_UNITS].join(', ')],
  ['Date', new Date()],
];
...
const getRanges = headers => {
  const [unit = 'none', ranges] = headers.get('Range')?.split(';')[0].split('=') ?? [];
  return { unit, ranges: ranges?.split(',').map($ => $.trim()) ?? [] };
};
