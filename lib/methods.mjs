...
const GET = async (url, { headers, integrity, signal }) => {
  ...
  const conditionalResponse = await tryConditionalResponse(headers, stats);
  ...
  switch (unit) {
  case 'bytes':
    return readRanges(url, ranges, stats.size)
...
const POSTorPUT = async (url, { body, headers, method }) => {
  ...
  return unit === 'none' || unit === 'bytes'
    ? writeRange(url, range ?? '-', body, flag).then(() => genericResponse(201))
    : genericResponse(416);
};
