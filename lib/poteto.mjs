...
const getFileURL = (url, fileURL = null) =>
  (fileURL = new URL(url = url instanceof Request ? url.url : url, cwdURL)).protocol === 'file:'
    ? fileURL
    : url;
...
const mergeOptions = (resource, options) => {
  if (!(resource instanceof Request))
    return options;
  const requestInit = {};
  requestInitOptions.forEach(key => requestInit[key] = resource[key]);
  return { ...requestInit, ...options };
};

class PotetoRequest extends Request {
  constructor(resource, options) {
    super(getFileURL(resource), mergeOptions(resource, options));
  }
}

/* TODO: use this in polyfill version
const ProxyRequest = new Proxy(Request, {
  construct: (target, [ resource, options ]) =>
...
const fileRequestURL = ([ resource, options ]) => (
  { url: getFileURL(resource instanceof Request ? resource.url : resource), request: new PotetoRequest(resource, options) });
...
const unbound = async (_, ...$) => {
  const { url, request } = fileRequestURL($);
